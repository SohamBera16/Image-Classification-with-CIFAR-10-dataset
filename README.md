...
## Model generation:
Using the layers in torch.nn (which has been imported as nn) and the torch.nn.functional module (imported as F), a neural network has been constructed based on the parameters of the dataset. Various Then the NLL loss function and an optimizer - Adam were created for the model training. 
